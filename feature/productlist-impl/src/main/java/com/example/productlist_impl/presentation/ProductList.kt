...
internal fun ProductListScreen(
    ...
    viewModel: ProductListViewModel? = null//TODO
) {
    val state = remember {
        viewModel?.state
    }
    LazyColumn(Modifier.padding(bottom = 48.0.dp)) {
        state?.value?.productList?.forEach {
            item {
                productItem(it.title, it.price, it.image, it.description)
            }
        }
    }
}
...
@Composable
fun productItem(title: String, price: String, url: String, description: String) {
    Card(
        modifier = Modifier
            .fillMaxWidth()
            .padding(16.dp),
